...
Warto przybliżyć również strukturę wyniku, który był zwracany przez zmodyfikowany algorytm. Funkcja zwracała tablicę wartości modularnośći dla każdego z kroków algorytmu. Ponadto uzupełniana była także tablica tablic results, która zawierała wyjściowy dendrogram. Po wykonaniu iteracji algorytmu do results dodawana była kolejna tablica, zawierającą krawędzie usunięte w danym kroku. W przypadku konstrukcji tej tablicy przyjęto następującą zasadę: gdy dochodziło do kopiowania wierzchołka pierwszą krawędź stanowiła pętla dla wierzchołka kopiowanego.\\
...
Algorytm OCDLCE został w pełni samodzielnie zaimplementowany. W celu łatwiejszych modyfikacji został on podzielony na 3 funkcje, odpowiadające jego krokom. Dwa pierwsze kroki zwracają graf oraz listę społeczności, zaś trzeci jedynie końcowy graf. \\
...
\\ Algorytm rozpoczyna się od pojedynczego przebiegu algorytmu dla społeczności rozłącznych. Jego wynikowa tablica jest tłumaczona tak aby numery społeczności były liczbami od $0$ do $k-1$ ($k$ -ilość społeczności w wyznaczonym podziale). Będzie ona podstawą do wygenerowania macierzy przynależności. Jest ona od razu aktualizowana. Do elementu $[i][j]$ dodajemy ilość połączeń wierzchołka $i$ z wierzchołkami ze społeczności $j$.\\
Następnie wywołujemy algorytm dla społeczności rozłącznych określoną liczbę razy. Po każdym przebiegu wynikowa tablica jest tłumaczona, tak by społeczności odpowiadające tym w tablicy z oryginalnego przebiegu miały ten sam numer. W tym celu dla każdej społeczności znajdowana jest inna, w której ilość pokrywających się wierzchołków jest największa. Następnie macierz przynależności jest aktualizowana, analogicznie jak po początkowym przebiegu.\\ 
...
wszystkich ludzi, którzy rozpoczęli swoją przygodę z programowaniem. Nie tylko ułatwia on pracę nad projektem w wieloosobowych zespołach, jednak potrafi być również wizytówką użytkownika. Okazało się, że może też stanowić źródło bardzo ciekawych danych. Portal ten udostępnia takie informacje jak: kto z kim pracował przy publicznych projektach, ile kontrybucji wykonał dany użytkownik, kiedy repozytorium zostało ostatnio zmodyfikowane.\\
...
Jak szybkie potrafią być algorytmy do wykrywania społeczności pokazała implementacja przy użyciu biblioteki CUDA. Rezultat znacząco przekroczył nasze oczekiwania. Co prawda implementacja ta jest częściowo okrojona, lecz z powodzeniem można ją rozszerzać o modyfikacje zaimplementowane w wersji na CPU. Oprócz danych uzyskanych za pomocą scraperów użyto tutaj zbioru z portalu \textit{Networkrepository}\cite{nr-aaai15}.
...
Poniżej przedstawiono wyniki działania dla modyfikacji związanej z datami zawarcia znajomości. O ile przy drugim grafie wynik nie wygląda lepiej niż w wersji bez modyfikacji, to w przypadku grafu mniejszego można zauważyć zdecydowanie bardziej naturalne rozłożenie etykiet. Również w przypadku grafu największego rozmiary społeczności stały się dużo bardziej zbalansowane - nie pojawiła sie społeczność, która zdecydowanie przeważałaby w grafie. Modyfikacja ta ma znaczący wpływ na wynik działania algorytmu i mogłaby z powodzeniem zostać użyta w praktycznej analizie problemu związanego z sieciami społecznościowymi. Ponadto zmienia fundamentalny aspekt algorytmu - rozstrzyganie remisów jest teraz deterministyczne, a w rezultacie cały algorytm zyskuje tę własność.
...
Otrzymane rezultaty sa znacząco inne niż w wersji synchronicznej. Społeczności jest zdecydowanie więcej, część z nich 'rozciąga się' przez znaczną część grafu. Małe kliki uformowały społeczności, co jest pożądanym rezultatem. Biorąc pod uwagę czas wykonania tej wersji algorytmu wyniki są bardzo zadowalające.\\

Interesujące rezultaty otrzymano badając kombinację algorytmu LPA z ostatnim krokiem algorytmu Louvain - łączeniem kilku różnych przyporządkowań społeczności rozłącznych w jedno przyporządkowanie nachodzące. Zastosowano tutaj wersję podstawową algorytmu LPA dla społeczności rozłącznych, ponieważ etap łączenia wymaga, aby rezultaty bazowego algorytmu były niedeterministyczne. Przykładowe wyniki eksperymentu przedstawiono poniżej.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/ms-louvain-lpa.png}
\caption{Wizualizacja kombinacji LPA + Louvain}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/ms-louvain-lpa-2.png}
\caption{Wizualizacja kombinacji LPA + Louvain}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/ms-louvain-lpa-3.png}
\caption{Wizualizacja kombinacji LPA + Louvain}
\end{figure}

Wyniki pokazują, że algorytm tworzy społeczności nachodzące tam, gdzie występował niedeterminizm. Wielkość przecięcia społeczności jest wiarygodna, ponadto można ją modyfikować za pomocą parametru. Wadą tego rozwiązania jest fakt, że etap łączenia ma złożoność wyższą, niż algorytm LPA, co uniemożliwia zastosowanie go dla grafów dużych. Jednak kombinacja ta jest wydajniejsza, niż użycie algorytmu Louvain w pierwszym kroku, co sprawia, że warto ją rozważyć w zastosowaniach praktycznych. W sprawozdaniu nie przedstawiono wyników połączenia algorytmu OCDLCE z algorytmem LPA, gdyż nie zmieniło ono rezultatów w zauważalny sposób.
...
\subsubsection{Analiza jakościowa}
W algorytmie przeprowadzono dwie modyfikacje i okazało się że obie mają istotny wpływ na rezultat. Wszystkie cztery przetestowane kombinacje zwracają rezultaty zbliżone do oczekiwanych.\\ 

Pierwszą zmianą było użycie innej miary modularności. Modularność f okazała się dużo lepszą miarą. Główną jej zaletą jest elastyczność związana z parametrem alpha. Dla wartości 1 modularność zachowuje się jak modularność M. Przetestowano dwie inne wartości: 0.5 i 1.5. Dla wartości 0.5 liczba znalezionych społeczności w pierwszym kroku okazała się nieznacznie mniejsza, zaś dla 1.5 kilka razy większa. Warto odnotować, że w kroku drugim różnica ta praktycznie zanika. Poniżej podane są przykładowe ilości społeczności po danym kroku dla grafu o 1391 wierzchołkach oraz wizualizację uzyskanych grafów:\\
...
Otrzymane rezultaty wyglądają całkiem podobnie, lecz wersja korzystającej z modularności f wydaje się wyznaczać lepsze rezultaty, przy użyciu podobnej liczby społeczności. To że rezultat jest lepszy można uzasadnić mniejszą liczbą wierzchołków należących do bardzo dużej liczby społeczności oraz większą liczbą należących do kilku społeczności, będących ich nakładającą się częścią.\\

Kolejną wprowadzoną modyfikacją była zmiana kroku pierwszego, polegającego na wyznaczaniu początkowych społeczności. Rezultaty okazały się znacznie odległe od oczekiwanych. Pierwszą intuicją związaną z ową modyfikacją było zmniejszenie liczby wierzchołków, gdyż znalezienie trzech wierzchołków niebędących aktualnie parami ze sobą w żadnej społeczności wydaje się trudniejsze od znalezienia takiej pary. Okazało się jednak, że liczba społeczności wzrosła kilkukrotnie. Po dłuższym zastanowieniu się można dojść do wniosku, że może wynikać to z faktu, że poprzez zaczęcie z 3 wierzchołków zamiast 2 początkowa modularność może być mniejsza. Kolejnym, niezbyt pożądanym efektem jest izolacja wierzchołków o małej liczbie sąsiadów. W związku z powyższym uzyskuje się graf o wielu początkowych społecznościach, względnie dużej liczbie wierzchołków mających przypisanych wiele społeczności oraz niepomijalnej liczbie wierzchołków, które nie mają przypisane żadnej. Poniżej znajdują się wyniki owej modyfikacji dla już wcześniej analizowanego grafu.\\
...
Temat społeczności nachodzących zachęca do analizy, posiadając dobrze zbilansowany próg wejścia do problemu. Jest to szerokie zagadnienie, które nie sposób opanować w jeden wieczór. Pomimo tego, wiedza, jaką trzeba posiadać, aby zacząć analizować i modyfikować algorytmy nie stanowi bariery, której forsowanie zajmuje dużą ilosć czasu. \\
