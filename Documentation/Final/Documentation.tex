...
Algorytm OCDLCE nie spełnił w pełni oczekiwań czasowych, już przy danych rzędu tysiąca trwał relatywnie długo. Ciekawą obserwacją, widzoczną na powyższym wykresie, jest fakt, że modyfikacje kroku pierwszego, które powinny być istotnie wolniejsze, nie mają dużego wpływu na rezultat. Modyfikacja modularności również nie wpłynęła istotnie na czas wykonania programu.\\

Z powyższych faktów można wywnioskować, że najbardziej kosztowny jest drugi krok algorytmu. Autorzy pracy naukowej, na której bazuje implementacja przekonywali, że pomimo teoretycznej złożoności kwadratowej, zazwyczaj krok wykonuje się dużo szybciej. Istnieje jednak możliwość, że faza łączenia społeczności nie została zaimplementowana w pełni optymalnie, co może mieć wpływ na uzyskany rezultat.\\

Innym powodem może być fakt, że w pracy, na której bazowano, zakładano liczbę krawędzi rzędu liczby wierzchołków, a rozpatrywane przez nas grafy były dużo bardziej gęste.\\

\subsubsection{Analiza jakościowa}
Algorytm zwraca bardzo ciekawe rezultaty. Gołym okiem można zauważyć, że dla wszystkich wersji algorytmu, niemal dla każdej pary społeczności istnieje duża grupa wierzchołków, należących do obu.\\

Modyfikacja modularności nie wpłynęła w istotny sposób na sposób wybierania społeczności. Najprawdopodobniej spowodowane jest to faktem, że jej równianie nie różni się bardzo od używanej wcześniej modularności M. W przypadku parametru alpha równego 1, liczba społeczności w była niemal zawsze taka sama, zaś dla innych wartości (na przykład 0.5) różniła się nieznacznie. 
Za równo w podstawowej wersji algorytmu, jak i tej używającej innej miary modularności wierzchołki należą do wielu społeczności i rzadko zdarza się sytuacja, aby jakiś wierzchołek nie miał przypisanej społeczności.\\


W przypadku modyfikacji kroku pierwszego, sytuacja wygląda zupełnie inaczej. Liczba początkowych społeczności jest dużo mniejsza, ale społeczności wydają się dużo bardziej zbalansowane, to znaczy zredukowana jest liczba zarówno wierzchołków należących do bardzo dużej ilości społeczności, jak i tych należących do jednej czy dwóch. Problemem jest zaś istotnie większa liczba wierzchołków nie należących do żadnej społeczności. Znajdują się one zazwyczaj na obrzeżach grafu. 
