...
\subsubsection{Analiza jakościowa}
Rezultaty zwracane przez algorytm są zadowalające. Gołym okiem można zauważyć, że dla wszystkich wersji algorytmu, niemal dla każdej pary społeczności istnieje duża grupa wierzchołków, należących do obu z nich.\\

Modyfikacja modularności nie wpłynęła w istotny sposób na zasadę wybierania społeczności. Najprawdopodobniej spowodowane jest to faktem, że jej równianie nie różni się bardzo od używanej wcześniej modularności M. W przypadku parametru alpha równego 1, liczba społeczności była niemal zawsze taka sama, zaś dla innych wartości (np. 0.5) różniła się nieznacznie. 
Zarówno w podstawowej wersji algorytmu, jak i tej używającej innej miary modularności, wierzchołki należą do wielu społeczności i rzadko zdarza się sytuacja, aby jakiś wierzchołek nie był przypisany do żadnej ze społeczności.\\

W przypadku modyfikacji kroku pierwszego, sytuacja wygląda zupełnie inaczej. Liczba początkowych społeczności jest dużo mniejsza, ale wynikowe społeczności wydają się dużo bardziej zbalansowane, to znaczy zredukowana jest liczba zarówno wierzchołków należących do bardzo dużej liczby społeczności, jak i tych należących do jednej czy dwóch. Problemem jest zaś istotnie większa liczba wierzchołków nie należących do żadnej społeczności. Znajdują się one zazwyczaj na obrzeżach grafu. 
