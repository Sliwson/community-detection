...
Algorytm OCDLCE został w pełni własnoręcznie zaimplementowany. W celu łatwiejszych modyfikacji został on podzielony na 3 funkcje, odpowiadające jego krokom. Dwa pierwsze kroki zwracają graf oraz listę społeczności, zaś 
trzeci jedynie końcowy graf. \\

W pierwszym kroku po kolei rozpatruje się wszystkie wierzchołki, a potem dla wybranego wierzchołka wszystkich jego sąsiadów, z którymi nie jest jeszcze w jednej społeczności. Potem zgodnie z algorytmem sprawdza się wspólnych sąsiadów i po zbadaniu modularności ewentualnie dodaje do tymczasowej społeczności. W przypadku gdy osiągnęła ona minimum 5 wierzchołków aktualizuje się zarówno informacje w wierzchołkach o przynależności do społeczności jak i listę wszystkich społeczności.\\

W drugim kroku łączy się społeczności. Napisano w tym celu specjalną funkcję Union, która za pomocą wyliczenia $WOS$ definiuje czy społeczności powinny być złączone. \\

W ostatnim kroku rozpatruje się wierzchołki nie należące do żadnej społeczności. W przypadku, gdy nie mają żadnej społeczności, rozpatruje się wszystkie społeczności sąsiadów, liczy modularność i w pozytywnym przypadku dodaje się rozpatrywany wierzchołek do danej społeczności.

\subsubsection{Podjęte decyzje}
Do pierwszego i trzeciego kroku dodano możliwość podania innej miary lokalnej modularności. W wersji podstawowej użyto modularności M, opisanej wzorem:\\ $M_in = \frac{M_in}{M_out}$ \\ zaś powyższa modyfikacja umożliwia zamiast tego skorzystanie z modularności f, opisanej wzorem:\\ $f_G = \frac{k^G_in}{(k^G_in + k^G_out)^\alpha}$\\, gdzie $k^G_in$ jest dwukrotnością liczby krawędzi o obu końcach w G, $k^G_out$ liczbą krawędzi o jednym końcu w G, zaś alpha parametrem. \\
...
wszystkich ludzi, mających choćby podstawowe doświadczenie z programowaniem. W znaczący sposób ułatwia on pracę nad projektem w wieloosobowych zespołach, jak i jest wizytówką użytkownika. Może też stanowić źródło bardzo ciekawych danych, udostępnia informację kto z kim pracował przy publicznych projektach. Dane te są całkiem łatwo dostępne za pośrednictwem api, problemem był jednak limit 5000 requestów na godzinę, przez co największy z otrzymanych grafów ma 1391 wierzchołków i 21695 krawędzi.

Do uzyskania grafu napisany został specjalny skrypt w języku Python, który przyjmuje początkowego użytkownika, a następnie w pętli dodaje na stos wszystkich innych użytkowników, z którymi wybrana osoba współpracowała przy danym projekcie, dodaje krawędź między rozpatrywaną osobą, a innym użytkownikiem i rozpatruje kolejną osobę ze stosu

Podsumowując, graf składa się z wierzchołków, którymi są użytkownicy GitHuba i krawędzi znajdujących się między tymi użytkownikami, którzy współpracowali ze sobą w publicznym repozytorium. Warty odnotowania jest fakt, że do krawędzi można dodać wagę taką jak data ostatniej edycji repozytorium, czy liczba wspólnych commitów obu użytkowników.
